...
### 3. Отделить runtime config от публичного шаблона

- добавить `config.example.yaml` без реальных user/chat IDs;
- перестать хранить реальный `config.yaml` в Git и добавить его в `.gitignore`;
- предусмотреть безопасную миграцию существующего VPS, чтобы обновление кода не затёрло рабочие настройки;
- документировать, какие поля являются локальными runtime values.
